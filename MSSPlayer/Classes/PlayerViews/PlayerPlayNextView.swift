...
public protocol PlayerPlayNextViewDelegate: class {
    func playNextView(_ playNextView: PlayerPlayNextView, didPlayNext playnext: Bool)
    func playNextView(_ playNextView: PlayerPlayNextView, didCancel cancel: Bool)
}

public protocol PlayerPlayNextView: UIView {
    // MARK: - Parameters
    var delegate: PlayerPlayNextViewDelegate? { get set }
    // MARK: - UI Display methods
    func show()
    func hide()
}

open class MSSPlayerPlayNextView: UIView, PlayerPlayNextView {
    ...
    open weak var delegate: PlayerPlayNextViewDelegate?
    ...
    @objc private func handleTap() {
        delegate?.playNextView(self, didPlayNext: true)
    }
    ...
    convenience public init() {
        self.init(size: CGSize(width: 40, height: 40), image: MSSImageResource.get(.mss_playNext))
    }
...
extension MSSPlayerPlayNextView: CAAnimationDelegate {
    public func animationDidStop(_ anim: CAAnimation, finished flag: Bool) {
        // flag: true mean completed
        if flag { delegate?.playNextView(self, didPlayNext: true) }
    }
}
